...
(async () => {
  try {
    const promptName = 'Proyectos industriales completos';
    const existing = await prisma.aiLearningPrompt.findUnique({ where: { name: promptName } });
    const systemPrompt = [
      'Eres un experto en soluciones industriales de ingeniería eléctrica.',
      'Usa los siguientes casos para responder con JSON estructurado de proyecto/soluciones/componentes.',
      'No inventes materiales ni servicios; mantente consistente con las marcas listadas.',
    ].join(' ');

    const contentExamples = examples
      .map(
        (example, index) =>
          `Caso ${index + 1} - Entrada:\n${JSON.stringify(example.input)}\nSalida:\n${JSON.stringify(example.output)}`,
      )
      .join('\n\n');

    const inputExample = JSON.stringify(examples[0].input);
    const outputExample = JSON.stringify(examples[0].output);

    if (existing) {
      await prisma.aiLearningPrompt.update({
      ...
      console.log('Updated existing prompt', promptName);
    } else {
      await prisma.aiLearningPrompt.create({
        data: {
          name: promptName,
