...
const outPath = 'apps/api/scripts/ai-training-inserts.sql';

const statements = dataset
  .map(
    (entry) => `INSERT INTO "TrainingExample" ("id","input","output","embedding","aceptado") VALUES (
gen_random_uuid(),
'${JSON.stringify(entry.input).replace(/'/g, "''")}'::jsonb,
'${JSON.stringify(entry.output).replace(/'/g, "''")}'::jsonb,
'{}'::double precision[],
true
);`,
  )
...
fs.writeFileSync(outPath, statements);
console.log('Wrote insert script to', outPath);
